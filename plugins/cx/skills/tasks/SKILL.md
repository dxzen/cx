...
2. 读取 `contract.md`、contract 引用的 `.cx/changes/<change-id>/specs/*.md`、可选 `design.md`、可选 `visual/style-guide.md`，并读取 `../core/protocols/common.md` 和 `../core/protocols/tasks.md`。
   ...
   - 不读取 `visual/prototype.editable.html` 作为任务拆分依据；它只是可编辑工作稿。
...
6. 写入 `.cx/changes/<change-id>/tasks.md`，必须复制 Tasks 协议骨架并保留 `## Requirement:`、checkbox、`RED:`、`GREEN:`、`VERIFY:`、`Command:`、`Expected failure:`、`Scope:` 等固定格式。
7. 运行：
...
8. 如果有 error，立即修复。
9. 成功后提醒用户：实现阶段只需要产物文件和相关代码，强烈建议执行 `/clear` 后继续 `build`。
