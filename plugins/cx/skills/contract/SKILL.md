...
目标：以资深产品经理的方式引导用户澄清需求，经过必要的多轮问答，把用户已有方向收敛为可实现、可验证、可交付的 `.cx/changes/<change-id>/contract.md`。
...
- 如果需求不足以写 Contract，进入资深 PM 澄清；每轮最多问 1-3 个会影响产品边界、实现方向或验收方式的问题。
...
- Related Durable Specs：是否已列出所有相关的 `.cx/specs/` 文件，并正确标注 unchanged/modified/added/removed/renamed 状态；unchanged 条目将成为下游回归验证的检查清单。
...
- 如果用户要求"你来定"，可以用明确假设继续推进，但必须在 Contract 中标注这些假设。
...
- `## Related Durable Specs` 必须以表格列出所有在 contract 阶段读取并判断为相关的 `.cx/specs/` 文件；每个文件标注状态（unchanged / modified / added / removed / renamed）。
- 状态为 modified/added/removed/renamed 的条目必须与 `## Spec Delta` 的 `Delta files:` 一致。
- 状态为 unchanged 的条目表示：该规格与本次变更相关，但行为不变；下游 verify 必须对其做回归验证。
- 如果不存在任何相关长期规格，写 `None`。
...
- 任何会约束实现或验收的产品结论都必须进入 `contract.md`，不要遗漏。
...
6. 创建 `.cx/changes/<change-id>/contract.md`，必须复制 Contract 协议骨架并填充 Intent、Scope、Requirements、Related Durable Specs、Spec Delta、Design Notes、Verification；Bug contract 必须包含 regression test 对应的 Requirement 或 Scenario。
6.5 写入 `## Related Durable Specs` 表格，列出所有相关 .cx/specs/ 文件及其状态；确保 modified/added/removed/renamed 条目与 `## Spec Delta` 的 Delta files 一致。如果没有相关长期规格，写 `None`。
...
10. 成功后提醒用户：Contract 已落盘，下游实现只依赖 `contract.md`、相关 change-local spec delta 和必要代码；建议执行 `/clear` 后继续 `visual`（如有 UI）或 `design`/`tasks`。
